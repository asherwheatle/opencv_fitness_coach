...
# ---------- GLOBAL TTS COOLDOWN STATE ----------
LAST_SPEAK_TIME = 0.0
SPEAK_COOLDOWN = 3.0  # 3 seconds cooldown
...
    """Speak text asynchronously (non-blocking) with a cooldown."""
    global LAST_SPEAK_TIME
    now = time.time()
    
    # --- COOLDOWN CHECK ADDED HERE ---
    if now - LAST_SPEAK_TIME < SPEAK_COOLDOWN:
        return # Exit, as cooldown is active
    # ---------------------------------
    
    # If not on cooldown, update the time and start the speech thread
    LAST_SPEAK_TIME = now
    
    def _speak():
        try:
...
    "plank":  {"body_straight_min_deg": 165}
}
...
def analyze_plank(lms,w,h,notes):
    sh,hp,an = get(lms,L.LEFT_SHOULDER,w,h),get(lms,L.LEFT_HIP,w,h),get(lms,L.LEFT_ANKLE,w,h)
    if not ok_vis(sh[2],hp[2],an[2]): return None,1.0
    body_ang=angle_3pt(sh,hp,an)
    form_score=1.0
    if body_ang<CFG["plank"]["body_straight_min_deg"]:
        form_score-=0.2
        msg="Raise hips slightly"
        notes.append(msg)
        speak_async(msg)
    return body_ang, form_score
